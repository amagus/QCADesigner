...
void create_main_window (main_W *main_window){
  ...
  GtkWidget
    ...
    *rotate_selection_menu_item      = NULL, *reset_selection_menu_item = NULL, *zoom_layer_button         	   = NULL,
    *zoom_in_menu_item               = NULL,
...
//Sardinha - RESET CELLS
  reset_selection_menu_item = gtk_image_menu_item_new_with_mnemonic (_("R_eset Selection"));
  gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (reset_selection_menu_item),
  img = gtk_image_new_from_stock (QCAD_STOCK_ROTATE_SELECTION, GTK_ICON_SIZE_MENU)) ;
  gtk_widget_show (img) ;
  gtk_widget_show (reset_selection_menu_item);
  gtk_container_add (GTK_CONTAINER (tools_menu_menu), reset_selection_menu_item);
...
//Sardinha
  gtk_widget_add_accelerator (reset_selection_menu_item ,       "activate", GTK_ACCEL_GROUP (accel_group),  GDK_F2,     0,                                  GTK_ACCEL_VISIBLE);
//END
...
//SARDINHA - Adding the cell reset callback function
  g_signal_connect (G_OBJECT (reset_selection_menu_item),                      "activate", (GCallback)reset_selection_menu_item_activate,            NULL);
//FIM SARDINHA
