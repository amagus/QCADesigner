...
//SARDINHA
void qcad_cell_reset (QCADCell *cell)
  {
	if (QCAD_CELL_FIXED != cell->cell_function)  	
		qcad_cell_set_polarization (cell, 0.0f);
  }
...
static void copy (QCADDesignObject *src, QCADDesignObject *dst)
  ...
  //Sardinha
  cellDst->cell_options.relax_in = cellSrc->cell_options.relax_in;
  cellDst->cell_options.relax_in_count = cellSrc->cell_options.relax_in_count; 	
  cellDst->cell_options.relax_cycles = cellSrc->cell_options.relax_cycles; 	
  cellDst->cell_options.relax_count = cellSrc->cell_options.relax_count;
  cellDst->cell_options.already_low = cellSrc->cell_options.already_low;
  cellDst->cell_options.old_mode = cellDst->cell_options.already_low;
...
static char *PostScript_instance (QCADDesignObject *obj, gboolean bColour)
  ...
  //Código alterado para gerar cores corretamente em escala de cinza. Por Sardinha.
  // Clock colours and their grayscale counterparts
  float fclrClock[4][2][3] = {
    {{0.20, 0.20, 0.20}, {0.45, 0.45, 0.45}}, // Clock 0: dark green
    {{0.40, 0.40, 0.40}, {0.65, 0.65, 0.65}}, // Clock 1: dark purple
    {{0.60, 0.60, 0.60}, {0.85, 0.85, 0.85}}, // Clock 2: turquoise
    {{0.80, 0.80, 0.80}, {0.95, 0.95, 0.95}}  // Clock 3: white
    } ;

  float fclrCellFunction[QCAD_CELL_LAST_FUNCTION][3] = {
    {0.00, 0.00, 0.00}, // QCAD_CELL_NORMAL: dark green (not used - clock takes precedence)
    {0.10, 0.10, 0.10}, // QCAD_CELL_INPUT:  dark azure blue
    {0.15, 0.15, 0.15}, // QCAD_CELL_OUTPUT: maroonish yellow
    {0.05, 0.05, 0.05}  // QCAD_CELL_FIXED:  dark orange
    } ;
//Fim alteração.
...
#ifdef UNDO_REDO
    if (NULL != pentry)
      state_before = qcad_design_object_get_state_array (obj, "function", "label", "polarization", "clock","relax","relax_in",NULL) ;
#endif /* def UNDO_REDO */
...
// This integer picks the current # of relaxed clocks.
    ...
    if (NULL != pentry)
      (*pentry) = qcad_design_object_get_state_undo_entry (obj, state_before,
        qcad_design_object_get_state_array (obj, "function", "label", "polarization", "clock","relax", NULL)) ;
#endif /* UNDO_REDO */
    }
...
static void create_properties_dialog (PROPERTIES *dialog)
  ...
  tblBig = gtk_table_new (4, 2, FALSE) ;
